...
if (-Not (az cognitiveservices account list -g $resourceGroup --query '[].name' -o json | ConvertFrom-Json) -Contains $name) {
    ...
    if (-Not (az cognitiveservices account list -g $resourceGroup --query '[].name' -o json | ConvertFrom-Json) -Contains $name) {
        Write-Error("The Azure OpenAI account $($name) was not found, and could not be created.")
        Pop-Location
        exit 1
    }
}
...
if (-Not ($deployments -Contains $completionsDeployment)) {
    ...
    $deployments = (az cognitiveservices account deployment list -g $resourceGroup -n $name --query '[].name' -o json | ConvertFrom-Json)
    if (-Not ($deployments -Contains $completionsDeployment)) {
        Write-Error("The Azure OpenAI deployment $($completionsDeployment) under account $($name) was not found, and could not be created.")
        Pop-Location
        exit 1
    }
}
...
if (-Not ($deployments -Contains $embeddingsDeployment)) {
    ...
    $deployments = (az cognitiveservices account deployment list -g $resourceGroup -n $name --query '[].name' -o json | ConvertFrom-Json)
    if (-Not ($deployments -Contains $embeddingsDeployment)) {
        Write-Error("The Azure OpenAI deployment $($embeddingsDeployment) under account $($name) was not found, and could not be created.")
        Pop-Location
        exit 1
    }
}
