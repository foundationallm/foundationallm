...
Param(
    ...
    [Parameter(Mandatory = $true)][string]$vecResourceGroup,
...
try {
    ...
    foreach ($container in (az storage container list --account-name $sourceStorageAccountName --query "@[].name" --auth-mode login -o tsv)) {
        azcopy copy "https://$($sourceStorageAccountName).blob.core.windows.net/$container/" "https://$destinationStorageAccount.blob.core.windows.net/backups/$storageResourceGroup/" --recursive
    }

    $sourceSearchServiceName = ""
    foreach ($resourceId in (az search service list -g $vecResourceGroup --query "@[].id" --output tsv)) {
        if ((az tag list --resource-id $resourceId --query "contains(keys(@.properties.tags), 'azd-env-name')") -eq $true) {
    ...
    if ([string]::IsNullOrEmpty($sourceSearchServiceName)) {
        throw "Could not find any search services with the azd-env-name tag in $vecResourceGroup."
    }

    # Save Search Indexes
    Get-SearchServiceIndexes -searchServiceName $sourceSearchServiceName -pathPrefix "https://$destinationStorageAccount.blob.core.windows.net/backups/$vecResourceGroup/data-sources/indexes"
}
catch {
