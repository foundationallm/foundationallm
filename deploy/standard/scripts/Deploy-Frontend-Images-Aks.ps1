...
if ($charts.Contains("chat-ui") -or  $charts.Contains("*")) {
    ...
    $command = "helm upgrade --install $name-web oci://ghcr.io/solliancenet/foundationallm/helm/chat-ui -f ../values/chat-ui-values.yml"
...
if ($charts.Contains("management-ui") -or  $charts.Contains("*")) {
    ...
    $command = "helm upgrade --install $name-management oci://ghcr.io/solliancenet/foundationallm/helm/management-ui -f ../values/management-ui-values.yml"
    $command = createHelmCommand $command
    Invoke-Expression "$command"
}
