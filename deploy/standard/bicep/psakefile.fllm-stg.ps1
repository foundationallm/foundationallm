...
task App -depends Agw, ResourceGroups, Ops, Networking, DNS {
    ...
    if($regenerateScripts) {
        Remove-Item "./app-rg.temp.ps1" -Force -ErrorAction Ignore
        ("az deployment group create --name " + $deployments['app'] + " ``
                            ...
                            --parameters actionGroupId=$script:actionGroupId ``
                                        ...
                                        vnetId=$script:vnetId") | Out-File -FilePath "./app-rg.temp.ps1"
    }

    Invoke-Expression "./app-rg.temp.ps1 $timestamp"
    if ($LASTEXITCODE -ne 0) {
...
task OpenAI -depends ResourceGroups, Ops, Networking, DNS {
    ...
    if($regenerateScripts){
        Remove-Item "./openai-rg.temp.ps1" -Force -ErrorAction Ignore
        (
            "az deployment group create --name " + $deployments['oai'] + " ``
                ...
                --parameters actionGroupId="+$script:actionGroupId + " ``
                             ...
                             privateDnsZones=" + $privateDnsZones + " ``
                             project=" + $project + " ``
                             vnetId=" + $script:vnetId) | Out-File -FilePath "./openai-rg.temp.ps1" -Force -Encoding ascii
    }

    Invoke-Expression "./openai-rg.temp.ps1 $timestamp"
    if ($LASTEXITCODE -ne 0) {
...
task Storage -depends ResourceGroups, Ops, Networking, DNS {
    ...
    $privateDnsZones = $($script:privateDnsZoneId | ConvertTo-Json -Compress)
    if ($regenerateScripts) {
        Remove-Item "./storage-rg.temp.ps1" -Force -ErrorAction Ignore
         ("az deployment group create --name " + $deployments['storage'] + " ``
              ...
              --parameters ``
                  ...
                  privateDnsZones=`'" + $privateDnsZones + "`' ``
                  project=" + $project + " ``
                  vnetId=" + $script:vnetId) | Out-File -FilePath "./storage-rg.temp.ps1" -Force -Encoding ascii
    }

    Invoke-Expression "./storage-rg.temp.ps1 $timestamp"
    if ($LASTEXITCODE -ne 0) {
