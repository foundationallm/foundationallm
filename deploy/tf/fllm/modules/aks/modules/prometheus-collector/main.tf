# Source: https://github.com/Azure/prometheus-collector/blob/b8d936bd826e1423fb94bd45ce8e812ef8266a3f/AddonTerraformTemplate/main.tf
# Licesne: MIT https://github.com/Azure/prometheus-collector/blob/b8d936bd826e1423fb94bd45ce8e812ef8266a3f/LICENSE
resource "azurerm_monitor_data_collection_endpoint" "dce" {
  name                = "MSProm-${var.resource_group.location}-${var.cluster.name}"
...
resource "azurerm_monitor_alert_prometheus_rule_group" "node_recording_rules_rule_group" {
  name                = "NodeRecordingRulesRuleGroup-${var.cluster.name}"
  location            = var.resource_group.location
  resource_group_name = var.resource_group.name
...
resource "azurerm_monitor_alert_prometheus_rule_group" "kubernetes_recording_rules_rule_group" {
  name                = "KubernetesRecordingRulesRuleGroup-${var.cluster.name}"
  location            = var.resource_group.location
  resource_group_name = var.resource_group.name
  cluster_name        = var.cluster.name
...
resource "azurerm_monitor_alert_prometheus_rule_group" "node_and_kubernetes_recording_rules_rule_group_win" {
  name                = "NodeAndKubernetesRecordingRulesRuleGroup-Win-${var.cluster.name}"
  location            = var.resource_group.location
  resource_group_name = var.resource_group.name
  cluster_name        = var.cluster.name
...
resource "azurerm_monitor_alert_prometheus_rule_group" "node_recording_rules_rule_group_win" {
  name                = "NodeRecordingRulesRuleGroup-Win-${var.cluster.name}"
  location            = var.resource_group.location
  resource_group_name = var.resource_group.name
  cluster_name        = var.cluster.name
  description         = "Node and Kubernetes Recording Rules Rule Group for Windows Nodes"
