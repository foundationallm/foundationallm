...
function Initialize-EntraIDAppRegistrations {

    $fllmAppRegs = @{}
    $ownerObjectIds = @()

    if (Test-Path  "$PSScriptRoot/../data/entra-app-registrations-admins.json" -PathType Leaf) {
        $ownerAccounts = Get-Content "$PSScriptRoot/../data/entra-app-registrations-admins.json" -Raw | ConvertFrom-Json
    ...
    if ((az ad app list --query "[?displayName=='$($params.fllmApi)']" -o tsv).Count -eq 0) {
        ...
        Write-Host "FoundationaLLM Core App Registrations already exist. Skipping creation."
    }
    ...
    $params = @{
     ...
     if ((az ad app list --query "[?displayName=='$($params.fllmApi)']" -o tsv).Count -eq 0) {
        ...
        Write-Host "FoundationaLLM Management App Registrations already exist. Skipping creation."
    }
    ...
    $params = @{
     ...
     if ((az ad app list --query "[?displayName=='$($params.fllmApi)']" -o tsv).Count -eq 0) {
        ...
        Write-Host "FoundationaLLM Authorization App Registration already exists. Skipping creation."
    }
