...
full_request_json_file_name = 'test/full_request_kql.json' # full original langchain request, contains agent, tools, exploded objects
...
kql_tool = foundationallmagent_tool_plugin_manager.create_tool(agent_tool, exploded_objects_json, user_identity, config)

#-------------------------------------------------------------------------------
# Direct tool invocation
response, content_artifacts = asyncio.run(kql_tool._arun(prompt='Get me the latest values in stock data'))
print("**** RESPONSE ****")
print(response)
