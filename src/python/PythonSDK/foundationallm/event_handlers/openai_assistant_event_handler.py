...
class OpenAIAssistantEventHandler(AssistantEventHandler):
    ...
    def __init__(self, client: AzureOpenAI, operations_manager: OperationsManager, request: OpenAIAssistantsAPIRequest, image_service: ImageService):
        super().__init__()
    ...
    def on_event(self, event: AssistantStreamEvent) -> None:
        if event.event == "thread.run.created":
            ...
            self.on_requires_action(event.data.id)
            ...
            if details and details.type == "tool_calls":
                for tool in details.tool_calls or []:
                    if tool.type == "code_interpreter" and tool.code_interpreter and tool.code_interpreter.input and tool.code_interpreter.input.endswith(tuple(self.stop_tokens)):
                        ...
                        self.update_state_api_analysis_results()
    ...
    def on_requires_action(self, run_id: str):
        max_steps = 20
        count = 0
                        ...
                        for call in run.required_action.submit_tool_outputs.tool_calls:
                            ...
                            if call.type == "function":
                                if call.function.name == "generate_image":
                                    try:
                                        tool_response = self.image_service.generate_image(**json.loads(call.function.arguments))
    ...
    def update_state_api_analysis_results(self):
        ...
        self.operations_manager.set_operation_result(self.request.operation_id, self.request.instance_id, self.interim_result)        

    def update_state_api_content(self):
