# Ensure the terminal working directory is Experimental
import asyncio
import json
import os
import sys
...
response = asyncio.run(
    graph.ainvoke(
    {'messages': messages},
    config={"configurable": {"original_user_prompt": user_prompt, **({"recursion_limit": agent.workflow.graph_recursion_limit} if agent.workflow.graph_recursion_limit is not None else {})}}
    )
)
#print("**** RESPONSE ****")
#print(response)
