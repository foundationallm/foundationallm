...
namespace FoundationaLLM
{
    ...
    {        
        ...
        /// <summary>
        /// Add the named <see cref="IStorageService"/> implementation for the FoundationaLLM.AzureAI resource provider.
        /// </summary>
        /// <param name="builder">The application builder.</param>
        public static void AddAzureAIResourceProviderStorage(this IHostApplicationBuilder builder)
        {
            builder.Services.AddOptions<BlobStorageServiceSettings>(
                DependencyInjectionKeys.FoundationaLLM_ResourceProviders_AzureAI)
                .Bind(builder.Configuration.GetSection(AppConfigurationKeySections.FoundationaLLM_ResourceProviders_AzureAI_Storage));

            builder.Services.AddSingleton<IStorageService, BlobStorageService>(sp =>
            {
                var settings = sp.GetRequiredService<IOptionsMonitor<BlobStorageServiceSettings>>()
                    .Get(DependencyInjectionKeys.FoundationaLLM_ResourceProviders_AzureAI);
                var logger = sp.GetRequiredService<ILogger<BlobStorageService>>();
                ...
                return new BlobStorageService(
                    ...
                    logger)
                {
                    InstanceName = DependencyInjectionKeys.FoundationaLLM_ResourceProviders_AzureAI
                };
            });
        }
        ...
        public static void AddAzureAIResourceProviderStorage(this IServiceCollection services, IConfiguration configuration)
        {
            services.AddOptions<BlobStorageServiceSettings>(
                DependencyInjectionKeys.FoundationaLLM_ResourceProviders_AzureAI)
                .Bind(configuration.GetSection(AppConfigurationKeySections.FoundationaLLM_ResourceProviders_AzureAI_Storage));
        ...
        /// <summary>
        /// Add the named <see cref="IStorageService"/> implementation for the FoundationaLLM.Vector resource provider.
        /// </summary>
        /// <param name="builder">The application builder.</param>
        public static void AddVectorResourceProviderStorage(this IHostApplicationBuilder builder)
        {
            builder.Services.AddOptions<BlobStorageServiceSettings>(
                DependencyInjectionKeys.FoundationaLLM_ResourceProviders_Vector)
                .Bind(builder.Configuration.GetSection(AppConfigurationKeySections.FoundationaLLM_ResourceProviders_Vector_Storage));

            builder.Services.AddSingleton<IStorageService, BlobStorageService>(sp =>
            {
                var settings = sp.GetRequiredService<IOptionsMonitor<BlobStorageServiceSettings>>()
                    .Get(DependencyInjectionKeys.FoundationaLLM_ResourceProviders_Vector);
                var logger = sp.GetRequiredService<ILogger<BlobStorageService>>();
                ...
                return new BlobStorageService(
                    ...
                    logger)
                {
                    InstanceName = DependencyInjectionKeys.FoundationaLLM_ResourceProviders_Vector
                };
            });
        }
        ...
        /// <param name="services">The <see cref="IServiceCollection"/> dependency injection container service collection.</param>
        /// <param name="configuration">The <see cref="IConfiguration"/> application configuration provider.</param>
        public static void AddVectorResourceProviderStorage(this IServiceCollection services, IConfiguration configuration)
        {
            services.AddOptions<BlobStorageServiceSettings>(
                DependencyInjectionKeys.FoundationaLLM_ResourceProviders_Vector)
                .Bind(configuration.GetSection(AppConfigurationKeySections.FoundationaLLM_ResourceProviders_Vector_Storage));
        ...
        /// <summary>
        /// Add the named <see cref="IStorageService"/> implementation for the FoundationaLLM.Context resource provider.
        /// </summary>
        /// <param name="builder">The application builder.</param>
        public static void AddContextResourceProviderStorage(this IHostApplicationBuilder builder)
        {
            builder.Services.AddOptions<BlobStorageServiceSettings>(
                DependencyInjectionKeys.FoundationaLLM_ResourceProviders_Context)
                .Bind(builder.Configuration.GetSection(AppConfigurationKeySections.FoundationaLLM_ResourceProviders_Context_Storage));

            builder.Services.AddSingleton<IStorageService, BlobStorageService>(sp =>
            {
                var settings = sp.GetRequiredService<IOptionsMonitor<BlobStorageServiceSettings>>()
                    .Get(DependencyInjectionKeys.FoundationaLLM_ResourceProviders_Context);
                var logger = sp.GetRequiredService<ILogger<BlobStorageService>>();
                ...
                return new BlobStorageService(
                    ...
                    logger)
                {
                    InstanceName = DependencyInjectionKeys.FoundationaLLM_ResourceProviders_Context
                };
            });
        }
        ...
        /// <param name="services">The <see cref="IServiceCollection"/> dependency injection container service collection.</param>
        /// <param name="configuration">The <see cref="IConfiguration"/> application configuration provider.</param>
        public static void AddContextResourceProviderStorage(this IServiceCollection services, IConfiguration configuration)
        {
            services.AddOptions<BlobStorageServiceSettings>(
                DependencyInjectionKeys.FoundationaLLM_ResourceProviders_Context)
                .Bind(configuration.GetSection(AppConfigurationKeySections.FoundationaLLM_ResourceProviders_Context_Storage));
        ...
        /// <summary>
        /// Add the named <see cref="IStorageService"/> implementation for the FoundationaLLM.Infrastructure resource provider.
        /// </summary>
        /// <param name="builder">The application builder.</param>
        public static void AddInfrastructureResourceProviderStorage(this IHostApplicationBuilder builder)
        {
            builder.Services.AddOptions<BlobStorageServiceSettings>(
                DependencyInjectionKeys.FoundationaLLM_ResourceProviders_Infrastructure)
                .Bind(builder.Configuration.GetSection(AppConfigurationKeySections.FoundationaLLM_ResourceProviders_Infrastructure_Storage));

            builder.Services.AddSingleton<IStorageService, BlobStorageService>(sp =>
            {
                var settings = sp.GetRequiredService<IOptionsMonitor<BlobStorageServiceSettings>>()
                    .Get(DependencyInjectionKeys.FoundationaLLM_ResourceProviders_Infrastructure);
                var logger = sp.GetRequiredService<ILogger<BlobStorageService>>();
                ...
                return new BlobStorageService(
                    ...
                    logger)
                {
                    InstanceName = DependencyInjectionKeys.FoundationaLLM_ResourceProviders_Infrastructure
                };
            });
        }
        ...
        /// <param name="services">The <see cref="IServiceCollection"/> dependency injection container service collection.</param>
        /// <param name="configuration">The <see cref="IConfiguration"/> application configuration provider.</param>
        public static void AddInfrastructureResourceProviderStorage(this IServiceCollection services, IConfiguration configuration)
        {
            services.AddOptions<BlobStorageServiceSettings>(
                DependencyInjectionKeys.FoundationaLLM_ResourceProviders_Infrastructure)
                .Bind(configuration.GetSection(AppConfigurationKeySections.FoundationaLLM_ResourceProviders_Infrastructure_Storage));
