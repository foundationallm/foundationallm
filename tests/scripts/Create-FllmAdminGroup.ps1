...
Param(
    [parameter(Mandatory = $false)][string]$groupName = "FLLM-DEMO-Admins",
...
try {
    ...
    $groupName = "FLLM-DEMO-Admins"

    # Build the command to create the Azure AD group using Azure CLI
    $createGroupCommand = "az ad group create --display-name $groupName --mail-nickname $groupName --output json"
