...
function New-FllmEntraIdApps {
    ...
    try {
        ...
        $appConfigUpdate = $appConfig | ConvertTo-Json -Depth 20 -Compress
        Set-Content -Path "$($fllmAppRegMetaData.Api.Name)`.json" -Value $appConfigUpdate
        ...
        if ($createClientApp) {     
            ...
            Set-Content -Path "$($fllmAppRegMetaData.Client.Name)`.json" -Value $appConfigUpdate
...
# Create FoundationaLLM Core App Registrations
$params = @{
    fllmApi              = "FoundationaLLM-DEMO"
    fllmClient           = "FoundationaLLM-DEMO-Client"
    fllmApiConfigPath    = "foundationalllm.json"
    fllmClientConfigPath = "foundationalllm-client.json"
    appPermissionsId     = "6da07102-bb6a-421d-a71e-dfdb6031d3d8"
    appUrl               = ""
...
$params = @{
    fllmApi              = "FoundationaLLM-Management-DEMO"
    fllmClient           = "FoundationaLLM-Management-DEMO-Client"
...
$params = @{
    fllmApi           = "FoundationaLLM-Authorization-DEMO"
