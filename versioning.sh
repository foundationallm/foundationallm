...
VERSION_REGEX='^(nuget-|pypi-|context_api-|user_portal-|core_api-|management_portal-|management_api-|langchain_api-|code_containers-)?'
VERSION_REGEX+='([0-9]+\.[0-9]+\.[0-9]+'
# allow optional pre-release like -rc485 or -rc485.1 (numeric, optional ".<num>")
# Changed numeric requirement to allow one or more digits (previously required 3+ digits)
VERSION_REGEX+='(-(alpha|beta|rc|poc|base|post)[0-9]+(\.[0-9]+)?)?'
VERSION_REGEX+=')$'
...
if [[ "$VERSION" =~ ^([0-9]+\.[0-9]+\.[0-9]+)-alpha([0-9]+)(\.[0-9]+)?$ ]]; then
